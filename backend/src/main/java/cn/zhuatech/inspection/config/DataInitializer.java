...
package cn.zhuatech.inspection.config;
import cn.zhuatech.inspection.domain.DomainCatalog;
import cn.zhuatech.inspection.model.*;
import cn.zhuatech.inspection.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "INSPECTION-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="INSPECTION-DEMO-"+String.format("%03d",sequence);
